...
재료 조건에 맞춰 자신의 인벤토리에 있는 선수들을 불러와 필터링해서 보여줌

예비선수 토글시 강화, 훈련레벨을 제외한 조건들을 필터링 해서 보여줌
...
육성선수 제외 토글시 훈련/강화중 한개 이상이 1이상 했을경우 필터링됨

선호구단 제회 토글시 현재 선호구단(이미지 내에선 한화)을 필터링

한번에 넣는게 아닌, 여러번에 걸쳐 넣을 수 있음.
...
넣은 선수의 총 합계가 10000 이상 채워져야 함. 한번에 넣는게 아닌, 여러번에 걸쳐 넣을 수 있음.
